...
import { products as SEED_PRODUCTS, categories as SEED_CATS } from '@/lib/mockData'
...
function initCatalogs() {
  ...
  const seeded = SEED_CATS.map((c, i) => ({ id: c.id, name: c.name, description: `${c.name} products` }))
...
export function StoreProvider({ children }) {
  ...
  useEffect(() => { save('catalogs', catalogs) }, [catalogs])
  useEffect(() => { save('products', products) }, [products])
  useEffect(() => { save('orders',   orders)   }, [orders])
  useEffect(() => { save('returns',  returns)  }, [returns])
  ...
  const addProduct = useCallback((data) => {
    const next = { id: Date.now(), rating: 4.5, sku: `PRD-${Date.now()}`, ...data }
  ...
  const addOrder = useCallback((orderData) => {
    const next = { ...orderData, createdAt: new Date().toISOString(), status: 'pending' }
  ...
  return (
    <StoreContext.Provider value={{
      // Catalogs
      catalogs, addCatalog, updateCatalog, deleteCatalog,
      // Products
