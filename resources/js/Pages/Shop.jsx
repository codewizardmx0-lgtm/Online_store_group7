...
function FilterSidebar({ isOpen, onClose, filters, setFilters, categories }) {
    return (
        <>
            {/* Mobile Overlay */}
            {isOpen && (
            ...
            >
                ...
                <Button
                    ...
                    className="w-full"
                    onClick={() =>
                        setFilters({ category: "all", price: "all", rating: 0 })
                    }
                >
                    Clear Filters
                </Button>
...
export default function Shop() {
    ...
    // Sync ?category= URL param into filter state
    useEffect(() => {
        const queryString = url.includes("?") ? url.split("?")[1] : "";
        const cat = new URLSearchParams(queryString).get("category");
        if (cat) {
            setFilters((prev) =>
                prev.category === cat.toLowerCase()
                    ? prev
                    : { ...prev, category: cat.toLowerCase() },
            );
        }
        ...
        .filter((product) => {
            ...
            if (filters.price !== "all") {
                ...
                if (max && (product.price < min || product.price > max))
                    return false;
                if (!max && filters.price === "200+" && product.price < 200)
                    return false;
            }
            return true;
        })
                        ...
                        <div className="flex items-center gap-4 ml-auto">
                            ...
                            <div className="relative hidden sm:block">
                                ...
                                <input
                                    ...
                                    className="h-9 pl-9 pr-3 w-52 rounded-[6px] border border-gray-200 text-sm bg-white focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
                                />
                            </div>
                            {/* Sort */}
