...
function StatCard({ title, value, sub, icon: Icon, color, to }) {
    const card = (
        <div
            className={`bg-white rounded-[14px] border border-gray-200 p-6 hover:shadow-md transition-shadow ${to ? "cursor-pointer" : ""}`}
        >
            <div className="flex items-center justify-between mb-4">
                <div
                    className={`w-10 h-10 rounded-[10px] flex items-center justify-center ${color}`}
                >
            ...
            <div className="grid sm:grid-cols-3 gap-6">
                            ...
                            >
                                {p.image ? (
                                    <img
                                        src={p.image}
                                        alt={p.name}
                                        className="w-8 h-8 rounded-[6px] object-cover bg-gray-100"
                                    />
                                ) : (
                                ...
                                <span
                                    className={`text-sm font-medium ${p.stock === 0 ? "text-red-600" : p.stock <= 10 ? "text-orange-600" : "text-gray-900"}`}
                                >
                                    {p.stock} in stock
                                </span>
                            </div>
                ...
                <div className="bg-white rounded-[14px] border border-gray-200 p-6">
                    ...
                    <div className="flex items-end gap-1.5 h-28">
                        {[40, 55, 35, 60, 48, 70, 52, 65, 44, 72, 58, 80].map(
                            (v, i) => (
                                <div
                                    key={i}
                                ...
                                >
                                    <div
                                        className="w-full bg-purple-600 rounded-t-[3px] hover:bg-purple-700 transition-colors"
                                        style={{ height: `${v}%` }}
                                    />
