...
export default function AdminReturns() {
  ...
  const handle = (id, status) => {
    ...
    addToast(`Return ${id} ${status}.`, status === 'approved' ? 'success' : 'info')
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">{returns.length} return request{returns.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      {returns.length === 0 ? (
        <div className="bg-white rounded-[14px] border border-gray-200 p-12 text-center">
          ...
          <p className="font-medium text-gray-900">No return requests</p>
          <p className="text-sm text-gray-500">Customer return requests will appear here.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {returns.map(r => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-mono text-xs text-gray-700">#{r.orderId}</td>
                  <td className="px-6 py-4 text-gray-700 max-w-[200px] truncate">{r.reason || '—'}</td>
